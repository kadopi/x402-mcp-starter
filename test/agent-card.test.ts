...
describe("x402 MCP Starter service card", () => {
  it("publishes payment-preflight discovery details without claiming A2A", async () => {
    ...
    const card = await response.json() as { mcpEndpoint: string; links: { productPage: string; githubReadme: string }; skills: Array<{ tags: string[]; examples: string[]; firstTool: string }>; a2a: { supported: boolean } };
    expect(response.status).toBe(200);
    expect(card.mcpEndpoint).toBe("https://x402-mcp-starter.kadopi.workers.dev/mcp");
    expect(card.links.productPage).toContain("/products/x402-mcp-starter");
    expect(card.links.githubReadme).toContain("x402-mcp-starter/blob/master/README.md");
